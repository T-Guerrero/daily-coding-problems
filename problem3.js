...
export default class Problem3 {
  ...
  static recursiveSerialize(node, s) {
    ...
    s += `${node.value}$`
    if (node.left !== null)
      s = Problem3.recursiveSerialize(node.left, s);
    else
      s += '*'
    
    if (node.right !== null)
      s = Problem3.recursiveSerialize(node.right, s);
    else
      s += '*'
    return s;
  }
  ...
  static recursiveDeserialize(s, index) {
    ...
    return new Node(value, Problem3.recursiveDeserialize(s, index), Problem3.recursiveDeserialize(s, index)) 
  }

  static tests = [
    () => {
      const root = new Node('root', new Node('left', new Node('left.left')), new Node('right'));
      return Problem3.deserialize(Problem3.serialize(root)).left.left.value === 'left.left';
    },
    () => {
      const root = null;
